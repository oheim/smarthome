...
def load_prices(start_day):
    timezone = pytz.timezone("Europe/Berlin")
    start = timezone.localize(datetime.datetime.combine(start_day, datetime.time()))
    start_ts = int(start.timestamp())
    now = datetime.datetime.now()
    today = now.date()
    end = timezone.localize(datetime.datetime.combine(today, datetime.time())) + datetime.timedelta(days=2)
    end_ts = int(end.timestamp())
    headers = {
        'Accept': 'application/json'
    }
    url = f"https://api.awattar.de/v1/marketdata?start={start_ts}000&end={end_ts}000"
    
    response = requests.get(url, headers=headers)
    response.raise_for_status()
    data = response.json()
    
    prices = {datetime.datetime.fromtimestamp(item["start_timestamp"] / 1000, timezone): item["marketprice"] for item in data["data"]}
...
def update_prices():
    ...
    try:
        now = datetime.datetime.now()
        today = now.date()
        if known_price_date is None:
            load_prices(today - datetime.timedelta(days=5))
        
        if known_price_date < today:
            load_prices(known_price_date)
